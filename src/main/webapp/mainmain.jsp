<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<%@ page import="newpackage.*"%>
<%@ page import="java.util.*"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
 <link href="adminhome.css" type="text/css" rel="stylesheet" />
 <!-- Boxicons CDN Link -->
 <link href='https://unpkg.com/boxicons@2.0.7/css/boxicons.min.css' rel='stylesheet'>
<title>home page</title>
<%
connectionDB condb = new connectionDB();
condb.getConnection();
%>
</head>



<body>
<%
         Date date = new Date();
        
String userid = (String)session.getAttribute("userid");
String serialnum = (String)session.getAttribute("serialnum");
String partnum = (String)session.getAttribute("partnum");
String status = (String)session.getAttribute("status");
String curr_step = (String)session.getAttribute("curr_step");

  String getprivilege = session.getAttribute("getprivilege").toString();
  String getjob = session.getAttribute("getjob").toString();

%>
<p id="hiddengetstep" hidden><%=curr_step%></p>


 <div class="sidebar">
    <div class="logo-details">
       <i class='bx bxs-label icon'></i>
        <div class="logo_name">MES</div>
        <i class='bx bx-menu' id="btn" ></i>
    </div>
    <ul class="nav-list">
   
      <li>
        <a href="homepage.jsp">
          <i class='bx bx-grid-alt'></i>
          <span class="links_name">Dashboard</span>
        </a>
         <span class="tooltip">Dashboard</span>
      </li>

<%
if (getprivilege.equals("f")) {
%>

<li>
        <a href="#">
          <i class='bx bx-pie-chart-alt-2' ></i>
          <span class="links_name">Output</span>
        </a>
         <span class="tooltip">Output</span>
      </li>

<%
}
%>

       <li class="profile">
        <div class="profile-details">
          <!--<img src="profile.jpg" alt="profileImg">-->
          <div class="name_job">
            <div class="name"><%=userid%></div>
            <div class="job"><%=getjob%></div>
          </div>
        </div>
        <i class='bx bx-log-out' onclick="location.href ='logout.jsp';" id="log_out" ></i>
    </li>

    </ul>
  </div>

<section class="home-section">

 <div class="topnav">
        <a class="floatleft" href="homepage.jsp">
                    <img src="https://cdn.shopify.com/s/files/1/0467/8660/9318/files/Tokimeku_Tag_Tokimeku_Logo_480x480.png?v=1620028225" class="paddingimg" alt="tokimeku" style="width:200px;height:75px;">
                  </a>

        <p class="floatleft">Unit Information</p>
    
        </div>

<div class="container">
<form name="insertform" action="insert.jsp" method ="post">
SerialNumber :<input type="text" name="serialNum" value="<%=serialnum%>" readonly/><br>
PartNumber :<input type="text" name="partNum" value="<%=partnum%>" readonly/><br>

<%
if (status.equals("inprocess")) {
%>

Status :<input type="text" style="color:green;" name="status" value="<%=status%> " readonly/><br>
<%
} if (status.equals("onhold"))  {
%>
Status :<input type="text" style="color:red;" name="status" value="<%=status%> " readonly/><br>
<%
}
%>

Step :<input type="text" name="step" value="<%=curr_step%>" readonly/><br>

<%

if(status.equals("inprocess"))  {

if (curr_step.equals("step1"))  {

%>
Parameter1: <input type="number" name="param1" id="param1" step="any" /><br>
Parameter2: <input type="number" name="param2" id="param2" step="any"/><br>
Parameter3: <input type="number" name="param3" id="param3" step="any"/><br>
<p id="1"></p>
<%
}

else if(curr_step.equals("step2"))  {

%>
Parameter1: <input type="number" name="param1" id="param1" step="any" /><br>
Parameter2: <input type="number" name="param2" id="param2" step="any"/><br>
<p id="1"></p>
<%
}
%>

<input type="button" onclick="submitForm()" value="Submit">
<%
}

%>

<input type="button" onClick="goBack()" value="Back">

</form>
<input type="button" onclick="location.href ='onhold.jsp';" value="OnHold or scrap unit">

</div>
</section>
<script>
let sidebar = document.querySelector(".sidebar");
let closeBtn = document.querySelector("#btn");
let searchBtn = document.querySelector(".bx-search");

closeBtn.addEventListener("click", ()=>{
  sidebar.classList.toggle("open");
  menuBtnChange();//calling the function(optional)
});

searchBtn.addEventListener("click", ()=>{ // Sidebar open when you click on the search iocn
  sidebar.classList.toggle("open");
  menuBtnChange(); //calling the function(optional)
});

// following are the code to change sidebar button(optional)
function menuBtnChange() {
 if(sidebar.classList.contains("open")){
   closeBtn.classList.replace("bx-menu", "bx-menu-alt-right");//replacing the iocns class
 }else {
   closeBtn.classList.replace("bx-menu-alt-right","bx-menu");//replacing the iocns class
 }
}


</script>


<script>
function take_value() {
 var part = document.forms["myform"]["partNum"].value;
var http;
 if (window.XMLHttpRequest)
    {// code for IE7+, Firefox, Chrome, Opera, Safari
    http=new XMLHttpRequest();
    }
    else
    {// code for IE6, IE5
    http=new ActiveXObject("Microsoft.XMLHTTP");
    }
 
try{

  http.onreadystatechange = function() {

        if (this.readyState == 4 && this.status == 200) {
                   
           document.getElementById("1").innerHTML = http.responseText;
}
  };
http.open("GET", "insert.jsp", true);
http.send();
}
catch(e)  {
  console.log("get error here============" + e);
}
}


   function goBack(){
       window.location = 'homepage.jsp';
   }

function submitForm() {
  var gethidden = document.getElementById("hiddengetstep").innerHTML;
  if(gethidden=="step1")  {
    var param1 = document.forms["insertform"]["param1"].value;
    var param2 = document.forms["insertform"]["param2"].value;
    var param3 = document.forms["insertform"]["param3"].value;

    if(param1 != "" && param2 != "" && param3 != "")  {
       document.forms["insertform"].submit();
    }
    else{
      alert("Please input parameter");
       
    }

  }
   else if(gethidden=="step2")  {
    var param1 = document.forms["insertform"]["param1"].value;
    var param2 = document.forms["insertform"]["param2"].value;

    if(param1 != "" && param2 != "")  {
       document.forms["insertform"].submit();
    }
    else{
      alert("Please input parameter");
       
    }

  }
 

}






</script>



</body>
</html>